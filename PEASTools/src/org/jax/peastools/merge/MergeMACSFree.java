...
public class MergeMACSFree {
	...
	public static void main(String[] args){
		try {
			MergeMACSFree mf = new MergeMACSFree(args[0]);
			mf.addAnnotatedFeatures(args[1]);
			mf.addInsertFeatures(args[2]);
			mf.addConservationFeatures(args[3]);
			mf.addDenovoMotifs(args[4]);
			mf.addCTCFMotifs(args[5]);
			mf.writePeaks(args[6], args.length > 7 && args[7].equals("MERGED"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public MergeMACSFree(String peakfile) throws IOException{
	...
	public void writePeaks(String out, boolean merged) throws IOException{
		...
		bw.write(PeakFeatures.getMACSFreeColumnLabels());
		if(merged){
		...
		for(int i = 0; i < chrs.length; i++){
			...
			while(pi < peaks.length){
				...
				bw.write(pf.getMACSFreeFeaturesLine());
				if(merged){
